// background.js (versão com integração da API do Gmail, nova verificação de segurança e refresh forçado)
...
chrome.runtime.onInstalled.addListener(() => {
    console.log("Notifica SEI instalado. Verificando e iniciando monitoramento.");
    checkForUpdates();
});

chrome.runtime.onStartup.addListener(() => {
    console.log("Navegador iniciado. Verificando e iniciando monitoramento.");
    checkForUpdates();
});
...
/**
 * Envia uma notificação por E-MAIL usando a API do Gmail.
 */
 ...
 * Modificada para incluir a nova lógica de refresh forçado.
 */
async function checkForUpdates() {
    ...
    try {
        const response = await chrome.tabs.sendMessage(seiTabId, { action: "forceRefreshAndGetData" });

        if (response.status === "not_safe_to_refresh") {
            console.log("Notifica SEI: Não é seguro fazer o refresh. Apenas a coleta de dados foi executada.");
        }
        ...
        if (error.message.includes("Could not establish connection")) {
            console.log("Notifica SEI: A aba do SEI foi fechada. Monitoramento continuará, mas não pode verificar agora.");
        } else {
            console.error("Notifica SEI: Erro inesperado ao verificar atualizações:", error);
        }
    }
}
